...
begin
   ...
   Vis_Plugin := GetProgDir + 'Data\Plugins\Vis\' + PluginList.Items[CurIndexNum];
...
begin
   ...
   if FindFirst(GetProgDir + 'Data\Plugins\Vis\vis_*.dll', 0, SearchRec) = 0 then
   begin
 ...
 // Reload vis plug-in if it is released.
   if NumVismod = 0 then
      ...
      begin
         ...
         Vis_Plugin := GetProgDir + 'Data\Plugins\Vis\' + PluginList.Items[SelectedIndexNum];
         LoadVisModule(Vis_Plugin, Visheader, Vismod, NumVismod, Form1.Handle);
         if NumVismod = 0 then
            exit;
      end else
...
begin
 ...
 // In such case it is difficult to know whether vis window is created.
   ...
   SelectedIndexNum := PluginList.ItemIndex;
   Vis_Plugin := GetProgDir + 'Plugins\Vis\' + PluginList.Items[SelectedIndexNum];
  ...
  { if CurVisWindowAttr.VisHandle <> 0 then   // Visualization is active ?
   ...
   BassPlayer1.RunVisPlugin(GetProgDir + 'Data\Plugins\Vis\' + PluginList.Items[SelectedIndexNum],
...
begin
   Vis_Plugin := GetProgDir + 'Data\Plugins\Vis\' + PluginList.Items[Index];
