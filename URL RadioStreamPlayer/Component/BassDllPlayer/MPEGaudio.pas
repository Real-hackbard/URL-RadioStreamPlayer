  ...
  TMPEGaudio = class(TObject)
    ...
    public
      ...
      function ReadFromTagStream(TagStream : pchar;
                                 StreamSize : Longword;
                                 TagVer : word;
                                 var MP3TagInfo : TMP3TagInfo): Boolean;      { Load tag }
...
function TMPEGaudio.ReadFromTagStream(TagStream : pchar;       { Load tag from tag stream }
                                      StreamSize : Longword;
                                      TagVer : word;
                                      var MP3TagInfo : TMP3TagInfo): Boolean;
begin
   if TagVer = 1 then
      if FID3v1.ReadFromTagStream(TagStream, MP3TagInfo) then
         result := true
      else
         result := false
   else if TagVer = 2 then
      if FID3v2.ReadFromTagStream(TagStream, StreamSize, MP3TagInfo) then
         result := true
      else
         result := false
   else
