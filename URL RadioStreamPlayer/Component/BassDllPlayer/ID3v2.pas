...
type
  ...
  TID3v2 = class(TObject)
    private
      ...
      FLanguage: string;
      FLink: string;
      FOrigArtist: string;     // Added for TBASSPlayer, by Silhwan Hyun 2005-03-29
   ...
   // Added function ReadFromTagStream for TBASSPlayer by Silhwan Hyun
      function ReadFromTagStream(TagStream : pchar;          { Load tag from tag stream }
                                 StreamSize : Longword;
      ...
      property OrigArtist: string read FOrigArtist write FSetOrigArtist; { Original Artist name }
  end;
...
procedure ReadFramesNew2(TagStream : pchar; var Tag: TagInfo);
var
  Frame: FrameHeaderNew;
  Data: array [1..500] of Char;
...
// This procedure is added for TBASSPlayer  by Silhwan Hyun
procedure ReadFramesOld2(TagStream : pchar; var Tag: TagInfo);
var
  Frame: FrameHeaderOld;
  Data: array [1..500] of Char;
...
begin
  ...
  try
    TagData.Position := 0;
 // * Add share mode "fmShareDenyWrite" to allow modification during playing by BASS.
 // Destination := TFileStream.Create(FileName, fmOpenReadWrite);
    Destination := TFileStream.Create(FileName, fmOpenReadWrite+fmShareDenyWrite);
...
// This function is added for TBASSPlayer by Silhwan Hyun
function TID3v2.ReadFromTagStream(TagStream : pchar;
                                  StreamSize : Longword;
                                  var MP3TagInfo : TMP3TagInfo): Boolean;
var
  Tag: TagInfo;
begin
   ...
   begin
      ...
      begin
         ...
         MP3TagInfo.Comment := ExtractText(Tag.Frame[7], true);
      end;
   end;
