...
uses
  ...
  Dialogs, StdCtrls, BassPlayer, PluginCtrl, ExtCtrls,
  ComCtrls, Buttons, XPMan, Menus, ShellApi;
...
var
  ...
  BlockWidth, HBlockGap, VLimit : integer;
  BackColor : TColor;
...
uses
  VisPluginCtrl, DSPPluginCtrl, AddonConfig, wa_ipc, WasabiAPI;
...
begin
  ...
  if (GetFocus = self.Handle) and (FOldWnd <> self.Handle) then
    begin DC := GetWindowDC(self.Handle); if DC <> 0 then try
    R := Rect(0,0,self.Width,self.Height);
    DrawFocusRect(DC,R); FOldWnd := self.Handle;
    finally ReleaseDC(self.Handle,DC); end; end;
end;
...
begin
   ...
   if s2 <> '' then
      OpenDialog1.Filter := OpenDialog1.Filter +
                            'Winamp plug-in supported files (' + s2 + ')|' + s2 + '|';
   if s3 <> '' then
      OpenDialog1.Filter := OpenDialog1.Filter +
                            'BASS add-on supported files (' + s3 + ')|' + s3 + '|' ;
...
begin
   BassPlayer1.Volume := ScrollBar1.Position;
...
begin
   ...
   begin
      SongPos := Trunc(TrackBar13.Position * BassPlayer1.PlayLength / TrackBar13.Max);
...
begin
   ...
   begin
      ...
      if not NowTracking and (BassPlayer1.Mode <> plmStopped) then
         if BassPlayer1.PlayLength > 0 then
            TrackBar13.Position := (BassPlayer1.Position * TrackBar13.Max) div BassPlayer1.PlayLength;
   end else
   ...
   Timer1.Enabled := true;
   Label22.Caption := IntToStr(TrackBar13.Position);

   if ProgressBar1.Position < 11000 then
    Label3.Caption := IntToStr(ProgressBar1.Position) + ' KBit/s'
   else
    Label3.Caption := 'Max KBit/s';
   ...
   case TPluginRequest(GenParam) of
      REQ_VOLUMEUP : begin
                           if ScrollBar1.Position < ScrollBar1.Max then
                              if (ScrollBar1.Max - ScrollBar1.Position) > 4 then
                                 ScrollBar1.Position := ScrollBar1.Position + 4
                              else
...
procedure TForm1.FormCanResize(Sender: TObject; var NewWidth,
  NewHeight: Integer; var Resize: Boolean);
begin
 // The vis_avs.dll plug-in alters form size at display mode change
...
begin
   BandNum := 1;
   EQGains[BandNum] := TrackBar2.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
begin
   BandNum := 2;
   EQGains[BandNum] := TrackBar3.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
begin
   BandNum := 3;
   EQGains[BandNum] := TrackBar4.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
begin
   BandNum := 4;
   EQGains[BandNum] := TrackBar5.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
begin
   BandNum := 5;
   EQGains[BandNum] := TrackBar6.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
begin
   BandNum := 6;
   EQGains[BandNum] := TrackBar7.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
begin
   BandNum := 7;
   EQGains[BandNum] := TrackBar8.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
begin
   BandNum := 8;
   EQGains[BandNum] := TrackBar9.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
begin
   BandNum := 9;
   EQGains[BandNum] := TrackBar10.Position;
   BassPlayer1.SetAEQGain(BandNum, EQGains[BandNum]);
end;
...
var
   ...
   R, G, B: Integer;
begin
  GaugePaintBox.Invalidate;
  GaugePaintBox.CleanupInstance;
  ...
  BasicBMP := TBitMap.Create;
   ...
   BasicBMP.Height := VLimit + 1;
   GaugeTempBMP := TBitMap.Create;
   GaugeTempBMP.Width := Panel1.Width-5;
   GaugeTempBMP.Height := Panel1.Height;
   GaugeRect.Left := 0;
   GaugeRect.Top := 0;
   ...
   begin
      Brush.Color := BackColor;
      Brush.Style := bsSolid;
      FillRect(Rect(0, 0, Panel1.Width-5, BasicBMP.Height));
      Pen.Color := clBlue;
      Pen.Width := 1;
      Pen.Style := psDot;
      MoveTo(0, 0);
      LineTo(0, BasicBMP.Height);        // Draw Y-axis line
      MoveTo(0, BasicBMP.Height - 1);
      LineTo(Panel1.Width-5, BasicBMP.Height - 1); // Draw X-axis line
   end;
...
begin
  for i := 1 to 10 do
     begin
        TTrackBar(findcomponent('TrackBar' + inttostr(i))).Position := 0;
     end;
  Panel1.SetFocus;
end;
...
begin
  ...
  PChar(ExtractFilePath(Application.ExeName) + 'URL\lst.ini'), nil, nil, SW_SHOWNORMAL);
  Panel1.SetFocus;
end;

procedure TForm1.V1Click(Sender: TObject);
begin
  btnShowVisCtrlForm.Click;
end;
