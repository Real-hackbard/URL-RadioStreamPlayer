  ...
  begin
    ...
    if IsExtraTag then
       if Info.ExtraTag = '' then
          Info.ExtraTag := FieldId + '=' + DecodeUTF8(Trim(FieldData))
       else  // use chr(0) as seperator between tags
          Info.ExtraTag := Info.ExtraTag + chr(0) + FieldId + '=' + DecodeUTF8(Trim(FieldData));
  end
  else
...
begin
  ...
  // * Added to get LPage information   by Silhwan Hyun
    Source.Seek(Info.TagEndPos, soFromBeginning);
    repeat
      Source.Read(Char4, 4);
      if Char4 = OGG_PAGE_ID then
      begin
         Info.LPagePos := Source.Position - 4;
         Source.Seek(Info.LPagePos, soFromBeginning);
         Source.Read(Info.LPage, SizeOf(Info.LPage));
         break;
      end else
         Source.Seek(Source.Position - 3, soFromBeginning);
    until Source.Position > (Source.Size - 3);
...
var
  ...
  tmpStr, TAGStr, FieldData: string;
     ...
     repeat
        ...
        begin
           FieldData := copy(TAGStr, 1, EqualPos)
                                  + EncodeUTF8(copy(TAGStr, EqualPos + 1, length(TAGStr) - EqualPos));
           Size := Length(FieldData);
           Result.Write(Size, SizeOf(Size));
           Result.WriteString(FieldData);
        end;
...
begin
  ...
  try
    { Create file streams }
    BufferName := FileName + '~';
  // * Add share mode "fmShareDenyWrite" to allow access during playing by BASS.
  //  Source := TFileStream.Create(FileName, fmOpenReadWrite+fmShareDenyWrite);
    ...
    Destination.CopyFrom(Tag, 0);  // Copy all contents of Tag

  // if it is possible to modify original source file directly then
  // ( This method is needed to allow access during playing by BASS )
   { if Destination.Size < Info.LPagePos then
    begin
       Source.Seek(0, soFromBeginning);
       Source.CopyFrom(Destination, 0);
       SetCRC(Source, Info);
       Source.Free;
       Destination.Free;
...
begin
  ...
  Result := (FChannelModeID in [VORBIS_CM_MONO, VORBIS_CM_STEREO, VORBIS_CM_3Ch,
                                VORBIS_CM_4Ch, VORBIS_CM_5Ch, VORBIS_CM_5Dot1Ch]) and
